...
<<'prepareAttachments', eval=TRUE, echo=TRUE>>=
attachFiles <- vector()
@
...
basert på data i NGER i perioden"
cap <- paste(cap, )
print(xtable::xtable(tabData$tabXXI))
@
...
\section{Kildedokument og figurer}
Utganspunktet for årsrapporten er automatisk fremstilling fra Rapporteket.
Vedlegg her kan brukes til videre mauelt arbeid med årsrapporten.

<<'prepareAnnotations', echo=TRUE, eval=TRUE>>=
...
if (usePdfAnnotation) {
  print('\\backmatter\n')
  print('\\section*{Kilder for videre bearbeidelse av årsrapporten}\n')
  print('Her er det satt av plass til nødvendige vedlegg.\n\n')
} else {
