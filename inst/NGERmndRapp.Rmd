---
title: Norsk gynekologisk endoskopiregister (NGER) månedsrapport for `r params$hospital`
...
RegData2aar <- nger::NGERPreprosess(
  RegData = nger::NGERRegDataSQL(datoFra = datoFra - 365, datoTil = datoTil)
)
...
maxDatoEget <- format.Date(
  max(RegDataEget$InnDato), format = "%d.%m.%Y", tz = "UTC"
)
```
...
antMnd <- 6
knitr::kable(nger::tabAntOpphShMnd(RegData=NGERData,
...
knitr::kable(nger::tabAntOpphShMnd(RegData=RegData2aar,
...
variabler <- c('KomplIntra', 'KomplPostop', 'KomplPostopAlvor', 'Opf0Reoperasjon')
for (valgtVar in variabler) {
	nger::NGERFigAndelTid(RegData=NGERData,
	                      valgtVar=valgtVar,
	                reshID=reshID, enhetsUtvalg=1,
              tidsenhet=tidsenhet, preprosess=0)
}
```
