...
if (!exists('RegData')){
      RegData <- nger::NGERRegDataSQL(datoFra = datoFra, datoTil = datoTil)
      RegData <- nger::NGERPreprosess(RegData=RegData)
...
RegDataEget <- RegData[which(RegData$ReshId==reshID), ]
maxDatoEget <- format.Date(max(RegDataEget$InnDato), format='%d.%m.%Y', tz='UTC')
@

<<'Testing1', include=FALSE>>=

   # NGERUtvalg <- NGERUtvalgEnh(RegData = RegData, reshID=reshID, datoFra = datoFra,
   #                             enhetsUtvalg=1)
...
<<'Testing2', include=FALSE>>=
nger::NGERFigAndelerGrVar(RegData=RegData, valgtVar='Alder', preprosess=0, datoFra=datoFra,
                datoTil=datoTil, outfile='Test.pdf', reshID=reshID)
 # nger::NGERFigKvalInd(RegData=RegData, preprosess=0, reshID=0,
 #                      datoFra = datoFra,
 #                      valgtVar='kvalInd',
...
# nger::NGERFigKvalInd(RegData=RegData, preprosess=0, reshID=reshID,
#                              valgtVar='kvalInd', enhetsUtvalg=1,
...
for (valgtVar in variable) {
	...
	nger::NGERFigAndelerGrVar(RegData=RegData, valgtVar=valgtVar, preprosess=0, datoFra=datoFra,
                datoTil=datoTil, outfile=outfile, reshID=reshID)
}
...
for (valgtVar in variabler) {
	...
	nger::NGERFigAndelTid(RegData=RegData, datoFra=datoFra, valgtVar=valgtVar,
	                datoTil=datoTil, reshID=reshID, enhetsUtvalg=1,
...
<<'AntMnd', results='asis'>>=
antMnd <- 6
xtable::xtable(nger::tabAntOpphShMnd(RegData=RegData, datoTil = Sys.Date(), antMnd=antMnd, reshID=reshID),
               digits=0,
               caption=paste0('Antall registreringer ved ditt sykehus siste ', antMnd, ' måneder.'))

xtable::xtable(nger::tabAntOpphShMnd(RegData=RegData2aar, datoTil = Sys.Date()-365, antMnd=antMnd, reshID=reshID),
               digits=0,
