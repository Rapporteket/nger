...
RegData2aar <- NGERPreprosess(NGERRegDataSQL(datoFra = (datoFra-365), datoTil = datoTil))
if (!exists('RegData')){
...
for (valgtVar in variable) {
	...
	NGERFigAndelerGrVar(RegData=RegData, valgtVar=valgtVar, preprosess=0, datoFra=datoFra,
...
for (valgtVar in variable) {
	...
	NGERFigAndelTid(RegData=RegData, datoFra=datoFra, valgtVar=valgtVar,
...
<<'AntMnd', results='asis'>>=
antMnd <- 6
xtable::xtable(tabAntOpphShMnd(RegData=RegData, datoTil = Sys.Date(), antMnd=antMnd, reshID=reshID),
               digits=0,
               caption=paste0('Antall registreringer ved ditt sykehus siste ', antMnd, ' måneder.'))

xtable::xtable(tabAntOpphShMnd(RegData=RegData2aar, datoTil = Sys.Date()-365, antMnd=antMnd, reshID=reshID),
               digits=0,
