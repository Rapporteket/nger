...
maxDatoEget <- format.Date(max(RegDataEget$OpDato), format='%d.%m.%Y', tz='UTC')
@
...
variabler <- c('KomplIntra', 'KomplPostop', 'Opf0Status')
for (valgtVar in variabler) {
	outfile <- paste0(valgtVar, '_Alle', tidsenhet, '.pdf')
	p <- nger::NGERFigAndelerGrVar(RegData=NGERData, valgtVar=valgtVar, preprosess=0,
