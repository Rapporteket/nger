...
Allepros <- sort(table(toupper(as.vector(as.matrix(NGERRegData[ ,c(ProsHys, ProsLap)])))), decreasing = TRUE)
AlleprosEget <- sort(table(toupper(as.vector(as.matrix(NGERRegData[indEget ,c(ProsHys, ProsLap)])))), decreasing = TRUE)
ant <- 20
...
dum <- NGERFigAndelTid(RegData=NGERRegData, datoFra=datoFra, valgtVar='Opf0Status', datoTil=datoTil,
