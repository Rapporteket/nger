...
datoTil <- sluttDato #Sys.Date()	#'2015-12-31'
startAar <- '2015-01-01' #Til bruk i årstabell
if (!exists('RegData')){
  RegData <- NGERRegDataSQL(datoFra = startAar, datoTil = datoTil)
}
@
...
if (tidsspenn > 300) {tidsenhet <- 'Aar'}
tidsperiodeTxt <- paste0('Operasjoner utført i perioden ', datoFra, ' til ', datoTil,'.' )
...
tabAvdN <- tabAntOpphSh5Aar(RegDataHele, datoTil=sluttDato)
...
\section{Pasientkarakteristikker og sykehusets aktivitet}
\tekstbf{HER BØR VI HA INN NOE MER FORKLARINGSTEKST.}
\par
...
TabPasKar <- NGERpasientegenskaper(RegDataHele[indEgetHele,])
...
print(xtable(AndelLapKomplTab, digits=c(0,1,1), align=c('l', 'l', rep('r', max(c(1,ncol(AndelLapKomplTab)-1), na.rm=T))),
             caption=paste0('Hyppighet av laparoskopiske komplikasjoner. ', tidsperiodeTxt,
                            ' Totalt ble det utført ', AntLap, ' i tidsperioden.'),
...
KonvEget <- table(RegDataLap[(indEgenLap %i% indKonv) , 'Aar'])/table(RegDataLap$Aar[indEget])*100
KonvTab <- rbind(
  'Konvertert' = Konv,
...
dum <- NGERFigAndelTid(RegData=RegData, preprosess=0, datoFra=datoFra, valgtVar='Opf0Status', datoTil=datoTil,
                       ...
                       OpMetode=OpMetode, Hastegrad=Hastegrad, tidsenhet = tidsenhet)
