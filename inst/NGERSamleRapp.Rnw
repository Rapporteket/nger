...
Denne samlerapporten er basert på registreringer som er lagret og ferdigstilt i NGER fra
...
#AlleprosSort <- sort(table(toupper(as.vector(as.matrix(RegData[ ,c(ProsHys, ProsLap)])))), decreasing = TRUE)

AlleProsEget <- toupper(as.vector(as.matrix(RegData[indEget, c(ProsHys, ProsLap)])))
...
Proc <- cbind( #Må fjerne tomme
  Andel = (AlleProsSort[1:ant])/N*100 ,
  Antall = AlleProsSort[1:ant] )
...
AlleDiagEget <- toupper(as.vector(as.matrix(RegData[indEget , c(DiagHys,DiagLap)])))
...
InstrTabDum <- plyr::ddply(RegDataUtvalg[ ,Instr], .(RegDataUtvalg$SykehusNavn), .drop=F, colwise(sum), na.rm=T)  #Dataramme m/7dim
Tot <- colSums(InstrTabDum[,2:(length(Instr)+1)])
ShNavn <- levels(InstrTabDum[,1])
...
#Få med resultat for eget sykehus.
RegDataLap <- RegData[which(RegData$OpMetode %in% c(1,3)), c('LapKonvertert','OpAar','ReshId')]
indKonv <- which(RegDataLap$LapKonvertert == 1)
Konv <- table(RegDataLap[indKonv,'OpAar'])/table(RegDataLap$OpAar[indMed])*100
KonvEget <- table(RegDataLap[(which(RegDataLap$ReshId == reshID) %i% indKonv) , 'OpAar'])/table(RegDataLap$OpAar[indEget])*100
#table(RegData$LapKonvertert[indMed,'Aar'])
KonvTab <- rbind(
  'Konvertert' = Konv,
...
dum <- NGERFigAndeler(RegData=RegData, datoFra=datoFra, valgtVar='LapTeknikk', datoTil=datoTil,
                      reshID=reshID, enhetsUtvalg=enhetsUtvalg, outfile='LapTeknikk.pdf', MCEType = 1,
                      minald=minald, maxald=maxald, Hastegrad=Hastegrad)
