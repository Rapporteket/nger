...
indEget <- which(NGERRegData$ReshId == reshID)
egetSh <- NGERRegData$SykehusNavn[match(reshID,NGERRegData$ReshId)]
N <- dim(NGERRegData)[1]
...
Alleproc <- sort(table(toupper(as.vector(as.matrix(NGERRegData[ ,c(ProcHys, ProcLap)])))), decreasing = TRUE)
AlleprocEget <- sort(table(toupper(as.vector(as.matrix(NGERRegData[indEget ,c(ProcHys, ProcLap)])))), decreasing = TRUE)
ant <- 20
Proc <- cbind( #Må fjerne tomme
          Andel = (Alleproc[2:ant+1])/N*100 ,
          Antall = Alleproc[2:ant+1] )

ProcEget <- cbind( #Må fjerne tomme
          Andel = (AlleprocEget[2:ant+1])/Neget*100 ,
          Antall = AlleprocEget[2:ant+1] )

Allediag <- sort(table(toupper(as.vector(as.matrix(NGERRegData[ ,c(DiagHys, DiagLap)])))), decreasing = TRUE)
AllediagEget <- sort(table(toupper(as.vector(as.matrix(NGERRegData[indEget ,c(DiagHys, DiagLap)])))), decreasing = TRUE)
ant <- 20
