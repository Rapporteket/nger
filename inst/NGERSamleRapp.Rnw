...
#Tabell <- table(RegData[,c('ShNavn', 'Aar')])
...
TabPasKar <- NGERpasientegenskaper(RegData)
...
visVanligsteProcDiag(RegData=RegData, ant=20, prosdiag='diag')
visVanligsteProcDiag(RegData=RegData, ant=20, prosdiag='pros')
...
for (met in 1:2) {
  ASALapHys <- addmargins(table(RegData[which(RegData$OpMetode == met), c('OpASA', 'Aar')]))
...
Konv <- table(RegDataLap[indKonv,'Aar'])/table(RegDataLap$Aar[indMed])*100
KonvEget <- table(RegDataLap[(which(RegDataLap$ReshId == reshID) %i% indKonv) , 'Aar'])/table(RegDataLap$Aar[indEget])*100
